...
Connects to Railway PostgreSQL via DATABASE_URL environment variable.
"""
...
async def add_wallet(user_id: int, amount: int, reason: str = "") -> None:
    """
    Add amount (may be negative) to wallet.
    Wallet can never go below 0.
    """
...
async def set_last_hourly(user_id: int, iso_datetime: str) -> None:
    await execute(
        "UPDATE users SET last_hourly = $1 WHERE user_id = $2",
        (iso_datetime, user_id),
    )
...
async def set_last_work(user_id: int, iso_datetime: str) -> None:
    await execute(
        "UPDATE users SET last_work = $1 WHERE user_id = $2",
        (iso_datetime, user_id),
    )
...
async def set_last_sidequest(user_id: int, iso_datetime: str) -> None:
    await execute(
        "UPDATE users SET last_sidequest = $1 WHERE user_id = $2",
        (iso_datetime, user_id),
    )
...
async def set_last_weekly(user_id: int, iso_datetime: str) -> None:
    await execute(
        "UPDATE users SET last_weekly = $1 WHERE user_id = $2",
        (iso_datetime, user_id),
    )
...
async def set_last_monthly(user_id: int, iso_datetime: str) -> None:
    await execute(
        "UPDATE users SET last_monthly = $1 WHERE user_id = $2",
        (iso_datetime, user_id),
    )
...
async def set_last_steal(user_id: int, iso_datetime: str) -> None:
    await execute(
        "UPDATE users SET last_steal = $1 WHERE user_id = $2",
        (iso_datetime, user_id),
    )
...
async def set_last_heist(user_id: int, iso_datetime: str) -> None:
    await execute(
        "UPDATE users SET last_heist = $1 WHERE user_id = $2",
        (iso_datetime, user_id),
    )


async def set_jail_until(user_id: int, iso_datetime: str | None) -> None:
    await execute(
        "UPDATE users SET jail_until = $1 WHERE user_id = $2",
        (iso_datetime, user_id),
    )
...
async def add_xp(user_id: int, xp_amount: int) -> int:
    """Add XP and auto-level. Returns new level."""
